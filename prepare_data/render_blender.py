...
for i in range(0, args.views):
    ...
    depth_file_output.file_slots[0].path = fp + '/{}_depth'.format(i)
    normal_file_output.file_slots[0].path = fp + '/{}_normal'.format(i)
    albedo_file_output.file_slots[0].path = fp + '/{}_albedo'.format(i)

    bpy.ops.render.render(write_still=True)  # render still

    b_empty.rotation_euler[2] += radians(stepsize)
    # np.savetxt(scene.render.filepath + ".npy", cam.matrix_world)